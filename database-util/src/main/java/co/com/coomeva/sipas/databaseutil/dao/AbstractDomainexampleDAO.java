...
public abstract class AbstractDomainexampleDAO<T> {
	...
	/**
	 * cceso restringido a nivel de clase
	 * @return
	 */
	...
	public QueryCustom getNamedQuery(String queryName) {
		return queryCustom.getNamedQuery(this.getCurrentSession(),queryName);
	}
	/**
	...
	public <T> List<Map<String,T>> getNamedQueryMap(String queryName) {	
		try {
			SQLQuery query = (SQLQuery)getCurrentSession().getNamedQuery(queryName);
